...
func TestRunList(t *testing.T) {
	...
	t.Run("categories conflicts with scheduled flag", func(t *testing.T) {
		cmd := &cobra.Command{}
		cmd.SetOut(&bytes.Buffer{})
		cmd.SetErr(&bytes.Buffer{})
	...
	t.Run("scheduled only shows articles with scheduledAt", func(t *testing.T) {
		dir := t.TempDir()
		writeMD(t, dir, "scheduled.md", "---\ntitle: Scheduled\ndate: 2026-04-01T00:00:00Z\ndraft: true\nscheduledAt: 2026-04-10T12:00:00Z\n---\n")
		writeMD(t, dir, "normal.md", "---\ntitle: Normal\ndate: 2026-03-01T00:00:00Z\ndraft: false\n---\n")
		...
		if strings.Contains(out, "Normal") {
			t.Errorf("expected Normal to be filtered out, got %q", out)
		}
	...
	t.Run("scheduled and draft conflict", func(t *testing.T) {
		cmd := &cobra.Command{}
		cmd.SetOut(&bytes.Buffer{})
		cmd.SetErr(&bytes.Buffer{})
		...
		if !strings.Contains(err.Error(), "--scheduled cannot be used") {
			t.Errorf("unexpected error message: %v", err)
		}
	})

	t.Run("scheduled and published conflict", func(t *testing.T) {
		cmd := &cobra.Command{}
		cmd.SetOut(&bytes.Buffer{})
		cmd.SetErr(&bytes.Buffer{})
	...
	t.Run("SCHEDULED_AT column is empty when no scheduledAt", func(t *testing.T) {
		dir := t.TempDir()
		writeMD(t, dir, "normal.md", "---\ntitle: Normal\ndate: 2026-03-01T00:00:00Z\ndraft: false\n---\n")
